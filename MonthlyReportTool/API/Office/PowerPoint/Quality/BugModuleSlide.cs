...
{
    public class BugModuleSlide : PowerPointSlideBase, IPowerPointQualitySlide
    {
        private PowerPointInterop.Slide slide;
        public BugModuleSlide(PowerPointInterop.Slide slide) : base(slide)
        {
            this.slide = slide;
        }
        ...
        private void DrawChart(List<BugEntity> allbugs)
        {
            ...
            object[] objHeaders = { "模块","BUG个数"};
            range = sheet.get_Range("A1", "B1");
            range.Value = objHeaders;

            var buglist = allbugs.GroupBy(bug => bug.ModulesName).OrderByDescending(bug => bug.Count());
