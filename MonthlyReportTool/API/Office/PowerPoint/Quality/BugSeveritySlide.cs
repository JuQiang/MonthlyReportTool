...
{
    public class BugSeveritySlide : PowerPointSlideBase, IPowerPointQualitySlide
    {
        private PowerPointInterop.Slide slide;
        public BugSeveritySlide(PowerPointInterop.Slide slide) : base(slide)
        {
            this.slide = slide;
        }
        ...
        private void DrawChart(List<BugEntity> allbugs)
        {
            ...
            object[] objHeaders = { "严重程度", "BUG个数" };
            range = sheet.get_Range("A1", "B1");
            range.Value = objHeaders;

            var buglist = allbugs.GroupBy(bug => bug.Severity).OrderBy(bug => bug.Key);
            ...
            chart.ChartTitle.Text = "BUG按等级分布情况";
            chart.Refresh();
            workbook.Close();
