...
namespace MonthlyReportTool.API.TFS.WorkItem
{
    ...
    {
        public static List<List<BacklogEntity>> GetAll(string project)
        {
            List<List<BacklogEntity>> list = new List<List<BacklogEntity>>();

            string iterationPath = Utility.GetBestIteration(project).Path;
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F05本迭代_已完成积压工作项总数"));
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F10本迭代_未启动积压工作项总数"));
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F15本迭代_已中止或已移除积压工作项总数"));            
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F00本迭代_积压工作项总数"));
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F30本迭代_已提交测试积压工作项总数"));
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F35本迭代_测试通过积压工作项总数"));
            list.Add(GetBacklogListByIteration(iterationPath, "共享查询%2F迭代总结数据查询%2F05%20Backlog统计分析%2F40本迭代_应提交测试积压工作项总数"));

            return list;
        }

        private static List<BacklogEntity> GetBacklogListByIteration(string iterationPath, string query)
        {
